...
class WriteFirebaseData {
    ...
    class func writePrompt(_ prompt: Prompt, completion: @escaping writePromptClosure) {
        ...
        FirebaseData.data.db.collection("courses").document("\(prompt.courseID)").collection("prompts").document("\(prompt.promptID)").setData([ "id": prompt.promptID, "title": prompt.promptTitle, "comment": prompt.promptComment, "courseId": prompt.courseID], merge: true,  completion: { error in
            if let error = error {
