                ...
                setUsername?.commitChanges(completion:
                    { (profileError) in
                        if profileError == nil {
                            ...
                            NotFirebaseAuth.addToKeychain(email: email, password: password)
                        }
                        else {
    ...
    class func loginWithTouchID(vc: UIViewController, email:String, completionHandler: @escaping (_ success: Bool) -> Void ) {
        ...
        DispatchQueue.global().async {
            do {
                ...
                NotFirebaseAuth.login(withEmail: email, password: password!, completionHandler: completionHandler)
            } catch let error {
                // Error handling if needed...
